...
router.post("/add", async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: "Games data posted successfully", data: insertedGames });
  } catch (error) {
...
router.put("/update/:id", async (req, res) => {
  ...
  try {
    const updatedGame = await GameModel.findByIdAndUpdate(gameId, updatedGameData, { new: true });
    if (!updatedGame) {
      return res.status(404).json({ message: "Game not found" });
    }
